    ...
    if (usePostgresApi()) {
        try {
            ...
            if (data.require2fa) {
                ...
                if (step2desc) step2desc.textContent = 'Masukkan kode 6 digit dari Google Authenticator / Authy';
                document.querySelectorAll('.otp-inp')[0]?.focus();
                return;
            }

            if (!res.ok) {
                showAlert(data.message || data.error || 'Login gagal.');
                return;
            }

            if (data.success && data.token) {
                localStorage.setItem('indo5_token', data.token);
                completeLogin(data.username || username, data.email || email, data.role || 'Administrator');
                return;
            }
...
function completeLogin(username, email, role) {
    ...
    setTimeout(() => {
        window.location.href = '/admin_dashboard_indo5.html';
    }, 800);
}
